...
function fetchPics(searchName) {
  ref.loadMore.hidden = true;
  ref.pageField.hidden = true;
  return fetch(`${URL}?key=${key}&q=${searchName}&page=${page}&per_page=${perPage}&image_type=photo&orientation=horizontal&safesearch=true`).then(resp => resp.json());
}
